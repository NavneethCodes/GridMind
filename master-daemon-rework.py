...
class MasterDaemon:
    ...
    def _send_worker_command(self, worker, payload):
        ...
        try:
            ...
            if not ack_raw:
                return False, 'no ack from worker'
    ...
    def compute_worker_score(self, worker):
        ...
        download = float(network.get('download_mbps', 0.0) or 0.0)
        upload = float(network.get('upload_mbps', 0.0) or 0.0)
        cores = float(benchmark.get('cpu_cores', 0) or 0)
        memory = float(benchmark.get('memory_gb', 0.0) or 0.0)
